...
def _is_entity_id_dict(entity_dict):
    # type: (dict) -> bool
    return (
        len(entity_dict.keys()) == 2
        and entity_dict.get("type")
        and entity_dict.get("id")
    )
...
def test_copy_context(populated_context):
    ...
    # test workfile
    new_context = populated_context.copy_context(
        workfile={"type": "workfile", "id": 123}
    )
    assert new_context.project == populated_context.project
    assert new_context.entity == populated_context.entity
    assert new_context.step == populated_context.step
...
def test_entity_dicts_equal():
    ...
    assert context._entity_dicts_equal(
        {"type": "project", "id": "123"}, {"type": "project", "id": "123"}
    )

    # not equal because of id
    assert not context._entity_dicts_equal(
        {"type": "project", "id": "13"}, {"type": "project", "id": "123"}
    )

    # not equal because of type
    assert not context._entity_dicts_equal(
        {"type": "projeect", "id": "123"}, {"type": "project", "id": "123"}
    )

    # not equal because of id and type
    assert not context._entity_dicts_equal(
        {"type": "projeect", "id": "13"}, {"type": "project", "id": "123"}
    )
...
def test_context__equal__(populated_context):
    ...
    assert not populated_context == populated_context.copy_context(
    ...
    assert not populated_context == populated_context.copy_context(
        task={"type": "project", "id": 123}
    )

    # test not matching workfile
    assert not populated_context == populated_context.copy_context(
        workfile={"type": "project", "id": 123}
    )

    # test not matching user
    assert not populated_context == populated_context.copy_context(
        user={"type": "project", "id": 123}
    )
...
def test_context__ne__(populated_context):
    ...
    assert populated_context != populated_context.copy_context(
