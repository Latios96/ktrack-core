...
def test_remove_bootstrapped_project(ktrack_instance_patched):
    ...
    for asset_name in data['asset_names']:
        asset = kt.create('asset', {'code': asset_name.replace(" ", ""),
                                    'asset_type': 'character',
                                    'project': project})
        entities.append(asset)

        for task_preset in task_presets['asset']:
            task = kt.create('task', {'project': project,
            ...
            workfile = kt.create('workfile', {'project': project,
                                              'entity': asset,
                                              'path': '123'})
    ...
    for shot_name in data['shot_names']:
        shot = kt.create('shot', {'code': shot_name.replace(" ", ""),
                                  'project': project})
    ...
    with mock.patch('os.walk') as mock_walk:
        ...
        with mock.patch('shutil.rmtree') as mock_rmtree:
            with mock.patch('kttk.path_cache_manager.unregister_path') as mock_unregister_path:
