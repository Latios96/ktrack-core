...
def cwd_hank():
    # type: () -> MagicMock
    with mock.patch('os.getcwd') as mock_get_cwd:
        mock_get_cwd.return_value = os.path.join(FINDING_DORY_PATH, 'Assets', 'character', 'Hank')
        yield mock_get_cwd
...
def project_has_asset_with_code(project_id, asset_code):
    ...
    assets = kt.find('asset', [['project', 'is', {'type': 'project', 'id': project_id}]])
...
def test_print_context_invalid_path(mock_print_result):
    ...
    mock_print_result.assert_called_once_with("No Context registered for path some_path")
...
class TestContextCommand(object):
    ...
    def test_invalid_path(mock_print_result):
        ktrack_command.print_context('some_path')
        mock_print_result.assert_called_with("No Context registered for path {}".format('some_path'))
...
class TestCreateCommand(object):
    ...
    @staticmethod
    def test_create_asset_no_asset_type(bootstrapped_project, mock_print_result, cwd_finding_dory):
    ...
    @staticmethod
    def test_create_task_no_step(bootstrapped_project, mock_print_result, cwd_finding_dory):
    ...
    @staticmethod
    def test_create_task_no_asset_shot(bootstrapped_project, mock_print_result, cwd_finding_dory):
...
class TestFindOneCommand(object):
    ...
    def test_find_non_existing_id(mock_print_result):
        ...
        mock_print_result.assert_called_with(
            'Entity of type "{}" and id "{}" not found..'.format('project', '5b6ef17e6e87ff1c2064aa24'))
    ...
    def test_find_non_existing_entity_type(mock_print_result):
        ...
        mock_print_result.assert_called_with("Entity type '{}' does not exist".format(entity_type))
