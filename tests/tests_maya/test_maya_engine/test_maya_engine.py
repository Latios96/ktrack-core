...
def test_current_file_path_saved_scene(maya_engine, saved_file):
    assert os.path.normpath(maya_engine.current_file_path()) == os.path.normpath(
        saved_file
    )
...
def test_open_file(maya_engine, workfile_dict, saved_file):
    ...
    with mock.patch(
        "kttk.engines.maya_engine.MayaEngine.change_file"
    ) as mock_change_file:
...
def test_save_as(maya_engine, saved_file, workfile_dict):
    ...
    with mock.patch(
        ...
        with mock.patch(
            "kttk.engines.maya_engine.MayaEngine.update_file_for_context"
        ) as mock_update_file:
...
def test_update_file_for_context(maya_engine, saved_file, populated_context, tmpdir):
    ...
    with patch.object(template_manager, "_data_routes", yml_data) as mock_yml_data:
        ...
        assert os.path.normpath(pm.workspace(q=True, rd=True)) == os.path.normpath(
...
def check_correct_file_state(context):
    ...
    assert (
        kttk.engines.current_engine().current_workfile["type"]
        == context.workfile["type"]
    )
    assert (
        kttk.engines.current_engine().current_workfile["id"] == context.workfile["id"]
    )
    ...
    assert kttk.engines.current_engine().deserialize_context_from_file().copy_context(
        user=None
    ) == context.copy_context(user=None)
