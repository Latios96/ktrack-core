...
class DataRetriver(object):
    ...
    def project_from_project_entity(self, task):
        return self._kt.find("project", [["id", "is", task["project"]["id"]]])[
            0
        ]  # todo use find_one

    def entity_from_task(self, task):
        return self._kt.find(
            str(task["entity"]["type"]), [["id", "is", task["entity"]["id"]]]
        )[
            0
        ]  # todo use find_one

    def entity_from_workfile(self, workfile):
        return self._kt.find_one(
            str(workfile["entity"]["type"]), workfile["entity"]["id"]
        )
...
class FileManagerWidget(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        ...
        self._task_list_view.selection_changed.connect(
            lambda indexes: self.task_selection_changed(indexes)
        )
        ...
        self._workfile_list_view.selection_changed.connect(
            lambda indexes: self.workfile_selection_changed(indexes)
        )
    ...
    def _advance(self):
        ...
        # update published files
        task = self.task_model.get_entity(
            self._task_list_view.selected_indexes()[0].row()
        )
        workfiles = self._data_retriver.get_workfiles(task)
        self.workfile_model.set_entities(workfiles)
